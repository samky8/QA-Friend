...
import { Globe, ChevronDown, ChevronUp, Link as LinkIcon, Eye, EyeOff } from "lucide-react";
...
const TargetWebpagePanel = ({ url, onUrlChange, auth, onAuthChange, ignoreHeaderFooter, onIgnoreHeaderFooterChange }: Props) => {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6">
      ...
      <button
        ...
        className="flex w-full items-center justify-between rounded-md border border-border bg-secondary/30 px-4 py-2.5 text-sm text-foreground transition-colors hover:bg-secondary/50"
      >
      ...
      {showAuth && (
        <div className="mt-3 space-y-3 rounded-md border border-border bg-secondary/20 p-4">
          <div>
          ...
          {auth.type !== "none" && (
            <>
              ...
              <p className="text-xs text-muted-foreground">
                🔒 Credentials are sent securely and never stored. They are cleared from memory after use.
              </p>
            </>
          )}
        </div>
